...
--not complete yet
create table checkin_month_year row format delimited fields terminated by '\t' stored as textfile location "/user/malam/yelp-1/checkin_month_year" as select business_id, year(timestamps) checkin_year, month(timestamps) checkin_month, count(business_id) checkin_counts from checkin1 group by business_id, timestamps;
